...
import '../shared/apartment_details_screen.dart';
...
class _MyApartmentsScreenState extends State<MyApartmentsScreen> with RealTimeRefreshMixin {
  ...
  void initState() {
    super.initState();
    _loadApartments();
    // Only start refresh for screens that need real-time updates
  ...
  Future<void> _toggleAvailability(String apartmentId, bool currentStatus) async {
    ...
    final apartmentIndex = _apartments.indexWhere((apt) => apt['id'].toString() == apartmentId);
    if (apartmentIndex != -1) {
      setState(() {
        _apartments[apartmentIndex]['is_available'] = !currentStatus;
        _apartments[apartmentIndex]['available'] = !currentStatus;
      });
    }
    ...
    try {
      ...
      if (result['success']) {
        ...
        // Only refresh if the optimistic update failed
      } else {
            ...
            children: [
              ClipRRect(
                ...
                child: images.isNotEmpty
                    ? Image.network(
                        AppConfig.getImageUrlSync(images.first),
                        height: 150,
