...
import '../../../core/utils/real_time_refresh_mixin.dart';
...
class _LandlordHomeScreenState extends State<LandlordHomeScreen> with TickerProviderStateMixin, RealTimeRefreshMixin {
  ...
  Future<void> _loadBookingRequests() async {
    try {
      ...
      if (result['success'] == true) {
        setState(() => _bookingRequests = List<Map<String, dynamic>>.from(result['data']['data'] ?? []));
      }
    } catch (e) {
      // Handle error
    }
                ...
                SafeArea(
                  child: Column(
                    children: [
                      ...
                      _buildTabBar(themeProvider.isDarkMode),
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [Color(0xFF4a90e2), Color(0xFFff6f2d)]),
              borderRadius: BorderRadius.circular(20),
            ),
            child: const Text('LANDLORD', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.white)),
          ),
          const Spacer(),
          Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              Text('${_myApartments.length} Properties', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppTheme.getTextColor(isDark))),
              Text('${_bookingRequests.length} Requests', style: TextStyle(fontSize: 12, color: AppTheme.getSubtextColor(isDark))),
            ],
          ),
  ...
  Widget _buildTab(String title, int index, bool isDark) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: isSelected ? const LinearGradient(colors: [Color(0xFF4a90e2), Color(0xFFff6f2d)]) : null,
          borderRadius: BorderRadius.circular(16),
        ),
        child: Text(
          title,
  ...
  Widget _buildMyApartments() {
    if (_myApartments.isEmpty) {
      return const Center(child: Text('No apartments listed', style: TextStyle(color: Colors.white)));
    }
    
    return ListView.builder(
      padding: const EdgeInsets.all(20),
      itemCount: _myApartments.length,
      itemBuilder: (context, index) => _buildApartmentCard(_myApartments[index]),
    );
  }
      ...
      children: [
        if (!apartment.isApproved)
          ApartmentStatusNotification(
            ...
            rejectionReason: null, // You can add rejection reason from backend if needed
          ),
        Container(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const Spacer(),
                        IconButton(
                          onPressed: apartment.isApproved ? () => _editApartment(apartment) : null,
                          icon: const Icon(Icons.edit, color: Colors.blue, size: 20),
                        ),
                        IconButton(
                          onPressed: () => _deleteApartment(apartment.id, apartment.title),
                          icon: const Icon(Icons.delete, color: Colors.red, size: 20),
                        ),
                        ElevatedButton(
                          onPressed: apartment.isApproved 
                              ? () async {
                                  await Navigator.push(context, MaterialPageRoute(builder: (_) => LandlordApartmentDetailsScreen(apartmentId: apartment.id)));
                                  _loadData();
                                }
                              : null,
                          style: ElevatedButton.styleFrom(
                            backgroundColor: apartment.isApproved ? const Color(0xFF4a90e2) : Colors.grey,
  ...
  Widget _buildBookingRequests() {
    if (_bookingRequests.isEmpty) {
      return const Center(child: Text('No booking requests', style: TextStyle(color: Colors.white)));
    }
    
    return ListView.builder(
      padding: const EdgeInsets.all(20),
      itemCount: _bookingRequests.length,
      itemBuilder: (context, index) => _buildBookingRequestCard(_bookingRequests[index]),
    );
  }
        ...
        children: [
          ...
          Row(
            children: [
              Text(
                '\$${request['total_price']}',
                style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFFff6f2d)),
              ),
              const Spacer(),
              ElevatedButton(
  ...
  Future<void> _approveRequest(String requestId) async {
    ...
    final requestIndex = _bookingRequests.indexWhere((req) => req['id'].toString() == requestId);
    if (requestIndex != -1) {
      setState(() {
    ...
    try {
      ...
      if (result['success']) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Booking request approved'), backgroundColor: Colors.green),
        );
  ...
  Future<void> _rejectRequest(String requestId) async {
    ...
    try {
      ...
      if (result['success']) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Booking request rejected'), backgroundColor: Colors.orange),
        );
        // Remove from pending list
        if (requestIndex != -1) {
          setState(() {
                ...
                errorBuilder: (context, error, stackTrace) => Container(
                  ...
                  child: const Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 8),
                      Text('Image not available', style: TextStyle(color: Colors.white, fontSize: 12)),
                    ],
                  ),
                ),
              )
            : Container(
                ...
                child: const Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 8),
                    Text('No images added', style: TextStyle(color: Colors.white, fontSize: 12)),
                  ],
                ),
              ),
      ),
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: RadialGradient(
                      colors: [
                        ...
                        const Color(0xFFff6f2d).withOpacity(isDark ? 0.2 : 0.05),
                        Colors.transparent,
                      ],
                    ),
                  ),
  ...
  Future<void> _editApartment(Apartment apartment) async {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Edit apartment feature coming soon'), backgroundColor: Colors.blue),
    );
  }
  ...
  Future<void> _deleteApartment(String apartmentId, String title) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: AppTheme.getCardColor(context.watch<ThemeProvider>().isDarkMode),
        title: Text('Delete Apartment', style: TextStyle(color: AppTheme.getTextColor(context.watch<ThemeProvider>().isDarkMode))),
        content: Text('Are you sure you want to delete "$title"?', style: TextStyle(color: AppTheme.getTextColor(context.watch<ThemeProvider>().isDarkMode))),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: Text('Cancel', style: TextStyle(color: AppTheme.getSubtextColor(context.watch<ThemeProvider>().isDarkMode))),
          ),
          TextButton(
    ...
    if (confirmed == true) {
      try {
        ...
        if (result['success']) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(result['message'] ?? 'Apartment deleted successfully'), backgroundColor: Colors.green),
          );
          _loadData();
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(result['message'] ?? 'Failed to delete apartment'), backgroundColor: Colors.red),
          );
        }
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to delete apartment'), backgroundColor: Colors.red),
        );
      }
    }
  }
